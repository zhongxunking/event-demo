...
import org.antframework.event.annotation.DomainListener;
import org.antframework.event.annotation.Listen;
...
public class UserListener2 {
    // 监听AddUserEvent事件
    // 结合@DomainListener(priority = 2)中priority属性，表示删除用户事件发生后本监听方法第二个执行
    ...
    // 结合@DomainListener(priority = 2)中priority属性，表示删除用户事件发生后本监听方法倒数第二个执行
